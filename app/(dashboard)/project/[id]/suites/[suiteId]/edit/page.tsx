...
import { X, GripVertical, Search, CheckCircle2, AlertCircle } from 'lucide-react'
...
export default function EditSuitePage() {
  ...
  const latestState = useRef({
    suiteName, description, cadence, ownerName, failThreshold, suiteTests, id, suiteId,
  })
  useEffect(() => {
    latestState.current = { suiteName, description, cadence, ownerName, failThreshold, suiteTests, id, suiteId }
  })

  useEffect(() => {
    Promise.all([
      fetch(`/api/projects/${id}`).then((r) => r.ok ? r.json() : null),
      fetch(`/api/projects/${id}/tests`).then((r) => r.json()),
      fetch(`/api/projects/${id}/areas`).then((r) => r.ok ? r.json() : { areas: [] }),
      fetch(`/api/suites/${suiteId}`).then((r) => r.json()),
    ]).then(([proj, tests, areasData, suiteData]) => {
      if (proj?.project?.name) setProjectName(proj.project.name)
      const testList: TestCase[] = tests.testCases || []
      setAllTests(testList)
      setAreas(areasData.areas ?? [])

      if (suiteData?.suite) {
        const s = suiteData.suite
        setSuiteName(s.name ?? '')
        setDescription(s.description ?? '')
        setCadence(s.cadence ?? 'EVERY_PR')
        setOwnerName(s.ownerName ?? '')
        setFailThreshold(s.failThreshold ?? 95)

        const orderedIds: string[] = (s.testCases ?? []).map(
          (stc: { testCase: TestCase }) => stc.testCase.id
        )
        const testMap: Record<string, TestCase> = {}
        testList.forEach((t) => { testMap[t.id] = t })
        setSuiteTests(orderedIds.map((tcId) => testMap[tcId]).filter(Boolean))
      }
      setLoading(false)
    })
  }, [id, suiteId])
  ...
  useEffect(() => {
    setActions(
      <div className="flex items-center gap-2">
        <Link href={`/project/${id}/suites/${suiteId}`}>
  ...
  }, [suiteName, suiteTests.length, submitting, isReady, id, suiteId])
  ...
  async function handleSave() {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`/api/suites/${s.suiteId}`, {
        method: 'PATCH',
        ...
        body: JSON.stringify({
          ...
          cadence: s.cadence,
          ownerName: s.ownerName.trim() || null,
      ...
      router.push(`/project/${s.id}/suites/${s.suiteId}`)
    } catch (err: unknown) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24 text-sm" style={{ color: 'var(--text-tertiary)' }}>
        Loading suite…
      </div>
    )
  }
    ...
    <div className="space-y-5 animate-fade-up">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
          Add or remove test cases, update cadence and ownership settings.
        </p>
      </div>
      ...
      <div className="flex gap-4" style={{ alignItems: 'flex-start' }}>
        ...
        <div
          ...
          style={{ border: '1px solid var(--border)', background: 'var(--surface-1)', minWidth: 0 }}
        >
          <div
            ...
            {visibleAreas.map((area) => (
              <button
                ...
                style={{
                  ...
                  background: areaFilter === area.id ? `${area.color}18` : 'var(--surface-2)',
            ...
            {(['P0', 'P1', 'P2', 'P3'] as const).map((p) => (
              <button
                ...
                style={{
                  ...
                  background: priorityFilter === p ? `${PRIORITY_COLORS[p]}18` : 'var(--surface-2)',
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); addTest(tc) }}
                    >
          ...
          >
            ...
            <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              {isReady
                ...
                : 'Add a suite name and at least one test case.'}
            </p>
          </div>
        </div>
      </div>
